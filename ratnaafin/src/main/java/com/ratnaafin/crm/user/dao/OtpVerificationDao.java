...
//    OtpVerificationDtl save(OtpVerificationDtl otpVerificationDtl);
    ...
    @Query("select u from otp_verification_dtl u where u.link_sent_status = 'P'")
    List<OtpVerificationDtl> findPendingOTPLinkDetail();

    @Modifying
    @Query("update otp_verification_dtl u set u.link_sent_status = ?2, u.link_sent_date = ?3  where u.token_id = ?1")
    void updateOTPLinkSentStatus(String token_id, String status, Date sentDate);
